...
bool MultiTextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Engine/source/multitexture.vs", L"../Engine/source/multitexture.ps");
	if(!result)
	{
		return false;
	}
...
bool MultiTextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, 
									 XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView** difusse, ID3D11ShaderResourceView** difusse1)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, difusse, difusse1);
	if(!result)
	{
		return false;
	}
...
bool MultiTextureShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
												  XMMATRIX projectionMatrix, ID3D11ShaderResourceView** difuse, ID3D11ShaderResourceView** difusse1)
{
	HRESULT result;
	...
	deviceContext->PSSetShaderResources(0, 1, difuse);
	deviceContext->PSSetShaderResources(1, 1, difusse1);
